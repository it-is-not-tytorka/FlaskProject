...
Удаление пользователя. Атрибут `status` пользователя помечается как `deleted`, после чего о нем нельзя получить новую инфомрацию, удаляются все папки, создалетем которых являлся пользователь.   
Responce example:
```html
<img src="link">
```
